...
team_t team={
    "team_belief",                                        /* team name */
    "Yingzhu Zhang",                                        /* first member full name */
    "yzhang21",                                        /* first member email address*/
...
void* mm_malloc (size_t size) {
  ...
  // If no more free blocks available:
    // Request for more memory to be put on heap
    // Then search the free list for free blocks again
...
void mm_free (void *ptr) {
  ...
  // Marks it as unused then adds it to the free list and 
  // Coalesces the free blocks together.
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  *(size_t *)UNSCALED_POINTER_ADD(newptr,reqSize - WORD_SIZE)=size;
  ...
  // Returns a pointer to an new allocated region
  return ptrNew;
}
